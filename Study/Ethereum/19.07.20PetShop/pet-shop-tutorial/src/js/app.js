...
if (window.ethereum) {
	...
	App.web3Provider = new Web3.providers.HttpProvider('http://your_geth_ip_address:8545');
...
App.contracts.Adoption.deployed().then(function(instance) {
	...
	for (i = 0; i < adopters.length; i++) {
		...
		//만약 i 번째 애완동물의 주인이 있을때

			$('.panel-pet').eq(i).find('button').text('Success').attr('disabled',true);

			//해당 버튼을 찾아서 Success로 글씨를 바꾼뒤 클릭이 안되도록 속성을 변경
...
web3.eth.getAccounts(function(error, accounts) {
	...
	App.contracts.Adoption.deployed().then(function(instance) {
		...
		return adoptionInstance.adopt(petId, {from: account,value:10000000000000});
